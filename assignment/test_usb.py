...
try:
    url = 'ftdi://ftdi:ft-x:D200B2IB/1'
    i2c.configure(url)  # Adjust URL as needed
except Exception as e:
    print(f"Error configuring I2C: {e}")
    exit(1)
...
# Initialize MPU6050 using direct I2C communication
from adafruit_mpu6050 import MPU6050
...
def read_angles():
    while True:
        try:
            ...
            # Print angles
            print(f"Angle X: {accel_x:.2f} degrees")
            print(f"Angle Y: {accel_y:.2f} degrees")
            print(f"Angle Z: {accel_z:.2f} degrees")
        except Exception as e:
